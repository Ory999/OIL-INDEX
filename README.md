...
Qualitative NLP Pipeline (cron 06:30 UTC, Mon–Fri)
  ...
  ├── Qual 2 Collect opec.py and collect eia steo.py    EIA STEO PDFs
  ├── Qual 3 Collect opec.py and build corpus.py        Merge into combined corpus
...
Note: the FinBERT baseline used in the paper (r = 0.13 vs `oil_impact_score`) is preserved as a frozen artefact in `data/Historic/finbert_scores.parquet`. It is not regenerated by the live pipeline.
...
python3 "scripts/qual/Qual 2 Collect opec.py and collect eia steo.py"
python3 "scripts/qual/Qual 3 Collect opec.py and build corpus.py"
