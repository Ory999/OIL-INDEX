# BERTopic topic clustering on institutional corpus.
# Pre-fitted model loaded from data/Historic/Bertopic, new documents predicted into existing topics.
# 9 topics, 7 substantive plus 2 noise filtered before momentum.
...
TOPIC_LABELS = {
    ...
    3:  "NOISE_EMAIL",       # cid, email, org, filtered
    ...
    8:  "NOISE_SOCIAL",      # capitalist, venezuela, visual, filtered
}
...
def ensure_model_available() -> bool:
    ...
    if HISTORIC_MODEL_PATH.exists():
        try:
            # dirs_exist_ok=True needed in case destination already exists.
            shutil.copytree(
