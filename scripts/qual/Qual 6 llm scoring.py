# LLM multidimensional scoring, gpt-oss-20b via ngrok tunnel.
# Resumable, historic backfill loaded from data/Historic, incremental save every 50 docs.
# SYSTEM_PROMPT and build_user_prompt locked to historic backfill, hash 0d23c19bbf76d4ee62cc685480ea43f0.
# reasoning_effort and RESPONSE_SCHEMA passed explicitly to match LM Studio preset behaviour.
# reasoning_effort falls back if model rejects it, preventing universal neutral scores.
# Do not modify SYSTEM_PROMPT, RESPONSE_SCHEMA, or build_user_prompt without re-scoring corpus.
...
# Flipped to False on first 400/422 error.
...
# Locked, hash 0d23c19bbf76d4ee62cc685480ea43f0.
...
def _make_api_call(client, model: str, messages: list,
    ...
    # Try with reasoning_effort, drop it on first parameter rejection.
    # Only OpenAI o1/o3 family supports it, local models return 400/422.
