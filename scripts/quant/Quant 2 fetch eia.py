# Fetch EIA weekly fundamentals via EIA Open Data API.
# Weekly inventory release is the #1 short-term oil price driver, must be controlled.
...
# Critical fundamental oil drivers.
EIA_SERIES = {
    # US crude stocks (Mbbl), inventory level
    "crude_stocks_mbbl":
    ...
    # US crude imports (Mbbl/day), supply diversity
    "crude_imports_mbpd":
...
def fetch_eia_fundamentals():
    ...
    # Week-on-week inventory change, market-moving surprise metric.
    df["crude_stocks_change"] = df["crude_stocks_mbbl"].diff()

    # EIA surprise, actual change minus 4-week rolling mean.
    # Positive surprise (unexpected build) is bearish, negative (unexpected draw) is bullish.
    df["eia_surprise"] = (
        df["crude_stocks_change"]
        - df["crude_stocks_change"].rolling(4, min_periods=1).mean()
    )

    # Weekly to daily resample.
