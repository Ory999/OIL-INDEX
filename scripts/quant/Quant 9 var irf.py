# VAR model and Impulse Response Functions on top Granger-significant features.
# Low dimensional VAR to avoid multicollinearity, max 5 variables.
...
def drop_correlated(df: pd.DataFrame, threshold: float = 0.90) -> pd.DataFrame:
    # Drop columns highly correlated with each other, keep TARGET.
...
def run_var_irf():
    ...
    # Drop correlated columns to prevent singular covariance.
    ...
    try:
        ...
        # Use selected_orders["aic"], not lag_select.aic.
