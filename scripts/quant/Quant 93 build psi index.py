# Build Price Sentiment Index for WTI, companion to PRCSI for divergence analysis.
# Scores current price movement against most extreme historical move in same direction.
# Three windows, 3-month structural, 1-week medium, 1-day tactical.
# Reference is expanding max rise / max fall since 2007, self-calibrating.
# Score 1.0 matches all-time record rise, 0.5 flat, 0.0 matches all-time record fall.
# Hybrid weights, price level dominates 2.5x, 3M 1.5x, 1W 0.75x, 1D 0.25x.
# Final composite EMA span 10 to match short price momentum decay.
...
# Shorter than PRCSI 63, price momentum decays in days not months.
EMA_SMOOTH  = 10
MIN_HISTORY = 252  # 1 year before expanding max/min

WINDOWS = {"3m": 63, "1w": 5, "1d": 1}

# Price level dominant, momentum modifies.
COMPONENT_WEIGHTS = {
    "price_fixed_rank": 2.5,  # is current price historically extreme
    "fg_3m":            1.5,  # 3M move extremity
    "fg_1w":            0.75, # recent week acceleration
    "fg_1d":            0.25, # single-day news reaction
}
...
def momentum_fear_greed(price: pd.Series, n_days: int, label: str) -> pd.Series:
    # Score n-day return against max historical move in same direction, bounded 0 to 1.
...
def build_psi():
    ...
    # Expanding percentile from 2007, every day judged against full history to that date.
    ...
    # Weighted composite and EMA smooth.
